...
{
    [EventSource(Name = "MyCompany-TinyURL-CommunicationAPI")]
    internal sealed class ServiceEventSource : EventSource
    {
        ...
        public void Message(string mes, params object[] args)
        {
            if (IsEnabled())
            {
                var finalMessage = string.Format(mes, args);
        ...
        {
            if (IsEnabled()) WriteEvent(MessageEventId, mes);
        }
 ...
 SizeInBytes(applicationTypeName) };
        ...
        private static long GetReplicaOrInstanceId(ServiceContext context)
        {
            ...
            if (stateless != null) return stateless.InstanceId;

            var stateful = context as StatefulServiceContext;
            if (stateful != null) return stateful.ReplicaId;
